...
class PublicTransport:
    """Метод для работы со свойствами общественного транспорта"""
    ...
    @property
    def info(self):
        """Выводить на печать информацию о: марке, цвете, годе выпуска и мощности двигателя"""

        return self.brand, self.color, self.year, self._engine_power


class Bus(PublicTransport):
    """Класс Bus с наследством от класса PublicTransport и доп.атрибутами"""
    ...
    @property
    def park(self):
        """Метод возвращает парк приписки автобуса"""

        return self.__park

    @park.setter
    def park(self, num_park):
        """Метод проверяет что добавляемое значение атрибута park находится в интервале от 1000 от 9999"""

        if 9999 >= num_park >= 1000:
            self.__park = num_park
        assert self.__park == num_park, 'Номер не входит в допустимый диапазон'


class Tram(PublicTransport):
    """Класс с наследством от класса PublicTransport и доп. атрибутами"""

    def __init__(self, brand, engine_power, year, color, max_speed, route, path, fare):
        super().__init__(brand, engine_power, year, color, max_speed)
        self.__route = route  # маршрут трамвая
        self.path = path  # длина маршрута
        self._fare = fare  # Стоимость проезда

    @property
    def how_long(self):
        """Метод вычисляет время на прохождение маршрута"""
